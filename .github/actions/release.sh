...
if [[ "${GITHUB_REF}" == refs/heads/main || "${GITHUB_REF}" == refs/tags/* ]]; then
    ...
    LIGHTTPD_BUILD_VERSION=$(curl -s https://download.lighttpd.net/lighttpd/releases-1.4.x/latest.txt | grep -oP 'lighttpd-\K[0-9]+\.[0-9]+\.[0-9]+' | sort -V | tail -n1)
    # Fetch the php version from docker hub
    docker pull php:${PHP_VERSION}-fpm-alpine
    PHP_BUILD_VERSION=$(docker inspect -f '{{ index .Config.Env }}' php:${PHP_VERSION}-fpm-alpine | grep -oP 'PHP_VERSION=\K[0-9]+\.[0-9]+\.[0-9]+' | sort -V | tail -n1)
    ...
    EXISTING_LIGHTTPD_BUILD_VERSION=$(docker inspect -f '{{ index .Config.Labels }}' siebsie23/php-lighttpd:${PHP_VERSION}-alpine | grep -oP 'LIGHTTPD_VERSION=\K[0-9]+\.[0-9]+\.[0-9]+' | sort -V | tail -n1)
    EXISTING_PHP_BUILD_VERSION=$(docker inspect -f '{{ index .Config.Labels }}' siebsie23/php-lighttpd:${PHP_VERSION}-alpine | grep -oP 'PHP_VERSION=\K[0-9]+\.[0-9]+\.[0-9]+' | sort -V | tail -n1)
